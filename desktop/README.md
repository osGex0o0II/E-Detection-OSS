...
The visual smoke test starts the published app, verifies the main window title, moves the main window to a stable size, captures a PNG under `artifacts\desktop\visual-smoke`, checks that the image is not blank or undersized, and writes a JSON result next to the screenshot. The settings smoke verifies persisted settings, startup provider status, settings schema migration, and the desktop health card. The single-instance smoke test starts the app hidden with `--background-startup`, verifies a duplicate background startup stays quiet, then verifies the legacy `--startup-minimized` compatibility entry restores the existing instance. The session-ending smoke test sends Windows session-ending messages to the main window, verifies a cancelled session does not close the app, then verifies a real end-session message exits cleanly. The startup-integration smoke toggles login startup from the settings UI, verifies the scheduled task or fallback startup entry points to the published app with `--background-startup`, then verifies disabling removes it. The environment-repair smoke creates a temporary venv, verifies `e_detection` is initially missing, runs the same local editable-install repair path, and verifies the module is importable afterward.
...
- Launching `EDetection.Desktop.exe --background-startup` starts the primary instance hidden in the tray for Windows login/background startup and keeps duplicate background launches quiet.
- The legacy `EDetection.Desktop.exe --startup-minimized` argument still starts hidden for compatibility, but duplicate launches with that argument restore the existing instance so user-triggered starts do not appear to do nothing.
...
- The "登录后自动启动" setting creates a per-user Task Scheduler logon task with `--background-startup`, a short logon delay, no battery-start restriction, and no 72-hour execution limit. Legacy `HKCU Run` entries are still recognized as a fallback and removed when Task Scheduler registration succeeds.
